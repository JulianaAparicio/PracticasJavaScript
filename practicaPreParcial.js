...
const inmobiliaria = {
  ...
  // B. Agregar un método listarDepartamentos que reciba como parámetro un array de departamentos y 
  ...
  listarDepartamentos: function (arrayDeptos) {
    for (let i = 0; i < arrayDeptos.length; i++) {
      ...
      let mascotas = arrayDeptos[i].aceptaMascotas
        ...
        : "no acepta mascotas";
      console.log(
        `id: ${arrayDeptos[i].id}, precio $ ${arrayDeptos[i].precioAlquiler}, 
        está ${disponible}, ${arrayDeptos[i].ambientes} ambientes, máximo ${arrayDeptos[i].cantidadPersonas} 
        personas, ${mascotas}, propietarios: ${arrayDeptos[i].propietarios}`
      );
    }
  },
  // C. Agregar un método departamentosDisponibles que permita consultar disponibilidad, es decir, que devuelva 
  //una lista de los departamentos disponibles.
  ...
  // E. Agregar un método buscarPorPrecio que permita filtrar los departamentos disponibles, siempre y 
  //cuando el precio sea menor o igual al precio enviado como argumento. 
